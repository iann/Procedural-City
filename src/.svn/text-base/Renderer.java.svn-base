...
class Renderer implements GLEventListener {
    ...
    private int[] textures = new int[6];			// Storage For 3 Textures
    ...
    private float z = -10.0f;			// Depth Into The Screen
    ...
    public void init(GLAutoDrawable gLDrawable) {
        ...
        try {
            texture = TextureReader.readTexture("data/images/grayBrick.png");
            texture2 = TextureReader.readTexture("data/images/mariobrick.png");
        } catch (IOException e) {
            e.printStackTrace();
            throw new RuntimeException(e);
        }

        gl.glGenTextures(6, this.textures, 0);
        ...
        gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_NEAREST);
        gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_NEAREST);
        makeRGBTexture(gl, glu, texture2, GL.GL_TEXTURE_2D, false);
